...
def bidirSync(
    ...
    if checkAccess:
        ...
        if rcloneLSL(localPathBase, localChkListFile, ['--include', chkFile], linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
            return RTN_ABORT

        if rcloneLSL(remotePathBase, remoteChkListFile, ['--include', chkFile], linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
            return RTN_ABORT

        status, localCheck = loadList(localChkListFile)
        if status:
            logging.error(printMsg("ERROR", "Failed loading local check list file <{}>".format(localChkListFile)))
            return RTN_CRITICAL

        status, remoteCheck = loadList(remoteChkListFile)
        if status:
            logging.error(printMsg("ERROR", "Failed loading remote check list file <{}>".format(remoteChkListFile)))
            return RTN_CRITICAL

        if len(localCheck) < 1 or len(localCheck) != len(remoteCheck):
            logging.error(
                printMsg("ERROR", "Failed access health test: <{}> local count {}, remote count {}".format(chkFile, len(localCheck), len(remoteCheck)), ""))
            return RTN_CRITICAL
        else:
    ...
    if rcloneLSL(localPathBase, localListFileNew, excludes, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
        return RTN_CRITICAL

    remoteListFileNew = listFileBase + '_remoteLSL_new'
    if rcloneLSL(remotePathBase, remoteListFileNew, excludes, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
        return RTN_CRITICAL
    ...
    if status:
        logging.error(printMsg("ERROR", "Failed loading prior local list file <{}>".format(localListFile)))
        return RTN_CRITICAL
    if len(localPrior) == 0:
        logging.error(printMsg("ERROR", "Zero length in prior local list file <{}>".format(localListFile)))
        return RTN_CRITICAL

    status, remotePrior = loadList(remoteListFile)
    if status:
        logging.error(printMsg("ERROR", "Failed loading prior remote list file <{}>".format(remoteListFile)))
        return RTN_CRITICAL

    if len(remotePrior) == 0:
        logging.error(printMsg("ERROR", "Zero length in prior remote list file <{}>".format(remoteListFile)))
        return RTN_CRITICAL

    status, localNow = loadList(localListFileNew)
    if status:
        logging.error(printMsg("ERROR", "Failed loading current local list file <{}>".format(localListFileNew)))
        return RTN_ABORT

    if len(localNow) == 0:
        logging.error(printMsg("ERROR", "Zero length in current local list file <{}>".format(localListFileNew)))
        return RTN_ABORT

    status, remoteNow =	loadList(remoteListFileNew)
    if status:
        logging.error(printMsg("ERROR", "Failed loading current remote list file <{}>".format(remoteListFileNew)))
        return RTN_ABORT

    if len(remoteNow) == 0:
        logging.error(printMsg("ERROR", "Zero length in current remote list file <{}>".format(remoteListFileNew)))
        return RTN_ABORT
    ...
    if len(localDeltas) > 0:
        ...
        for key in localDeltas:
            if localDeltas[key]['new']:
                news += 1
            if localDeltas[key]['newer']:
                newers += 1
            if localDeltas[key]['older']:
                olders += 1
        ...
        if remoteDeltas[key]['new']:
            ...
            if remoteDeltas[key]['newer']:
                if key not in localDeltas:
                    ...
                    dest = localPathBase + key 
                    logging.info(printMsg("REMOTE", "Copying to local", dest))
                    if rcloneCmd('copyto', src, dest, ["--ignore-times"] + switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
                        return RTN_CRITICAL
                else:
                    if key in localNow:
                        ...
                        if rcloneCmd('copyto', src, dest, ["--ignore-times"] +
                                     switches,
                                     linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
                            return RTN_CRITICAL
                        # Rename local
                        src = localPathBase + key 
                        dest = localPathBase + key + '_LOCAL' 
                        logging.warning(printMsg("LOCAL", "Renaming local copy", dest))
                        if rcloneCmd('moveto', src, dest, switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
                            return RTN_CRITICAL
                        else:
            ...
            if remoteDeltas[key]['deleted']:
                if key not in localDeltas:
                    if key in localNow:
                        ...
                        if rcloneCmd('delete', src, switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
                            return RTN_CRITICAL
    ...
    for key in localDeltas:
        if localDeltas[key]['deleted']:
            if (key in remoteDeltas) and (key in remoteNow):
                ...
                logging.warning(printMsg("REMOTE", "Copying to local", dest))
                if rcloneCmd('copyto', src, dest, switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
                    return RTN_CRITICAL
    ...
    if len(remoteDeltas) == 0 and len(localDeltas) == 0 and not first_sync:
        ...
        if rcloneCmd('sync', localPathBase, remotePathBase, excludes + switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
            return RTN_CRITICAL

        logging.info(">>>>> rmdirs Remote")
        if rcloneCmd('rmdirs', remotePathBase, None, switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
            return RTN_CRITICAL

        if not no_local_delete:
            logging.info(">>>>> rmdirs Local")
            if rcloneCmd('rmdirs', localPathBase, None, switches, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
                return RTN_CRITICAL
    ...
    if rcloneLSL(localPathBase, localListFile, excludes, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
        return RTN_CRITICAL

    if rcloneLSL(remotePathBase, remoteListFile, excludes, linenum=inspect.getframeinfo(inspect.currentframe()).lineno):
        return RTN_CRITICAL
